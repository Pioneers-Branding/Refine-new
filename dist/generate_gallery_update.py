...
html = """    <section class="py-16 lg:py-24 bg-surface-warm relative" id="gallery-section">
...
for vid in valid_videos:
    html += f"""                <!-- Video -->
                <div class="filter-item break-inside-avoid relative rounded-3xl overflow-hidden group bento-item shadow-lg" data-category="video" style="display: none;">
                    <video src="/{vid}" class="gallery-video w-full h-auto rounded-3xl cursor-pointer" controls preload="metadata"></video>
                </div>
"""
...
for img in images:
    html += f"""                <!-- Image -->
                <div class="filter-item break-inside-avoid relative rounded-3xl overflow-hidden group bento-item shadow-lg" data-category="image" style="display: none;">
                    <img src="/{img}" alt="Gallery Image" class="gallery-image w-full h-auto rounded-3xl cursor-pointer transform transition-transform duration-700 group-hover:scale-[1.02]" loading="lazy">
                </div>
"""
...
# Need to find the exact start marker. Let's use a dynamic approach.
start_idx = content.find('<section class="py-16 lg:py-24 bg-surface-warm relative"')
if start_idx == -1:
    start_idx = content.find('<section class="py-16 lg:py-24 bg-surface-warm relative">')
...
if start_idx != -1 and end_idx != -1:
    ...
    with open('gallery.php', 'w') as f:
        f.write(new_content)
    print("Gallery Show Less updated successfully")
else:
