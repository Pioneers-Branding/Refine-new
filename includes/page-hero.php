    ...
    <div class="max-w-[1600px] mx-auto px-6 lg:px-10 relative z-10">
        <div class="max-w-3xl gs-reveal-text">
            <?php if (isset($pageCategory)): ?>
                <span class="block text-accent font-body text-xs tracking-[0.25em] uppercase mb-4 font-semibold"><?php echo htmlspecialchars($pageCategory); ?></span>
            <?php endif; ?>
            
            <h1 class="text-section font-display text-brand-deeper leading-tight mb-6"><?php echo isset($pageTitle) ? $pageTitle : 'Refine Skin & Body Clinic'; ?></h1>
            
            <?php if (isset($pageDescription)): ?>
                <p class="text-brand-muted font-body text-lg lg:text-xl font-light leading-relaxed"><?php echo $pageDescription; ?></p>
            <?php endif; ?>
        </div>
    </div>
</section>
